...
let memoryStore = globalThis.__communMemoryStore
if (!memoryStore) {
  memoryStore = new Map()
  globalThis.__communMemoryStore = memoryStore
}

function cloneCommonTool(record) {
  if (!record) return null
  const normalized = {
    hash: normalizeHash(record.hash),
    name: sanitizeString(record.name),
    contact: sanitizeString(record.contact),
    weight: sanitizeString(record.weight),
    date: sanitizeString(record.date),
    lastUser: sanitizeString(record.lastUser),
    dimensions: sanitizeString(record.dimensions),
    lastScanAt: normalizeNullable(record.lastScanAt),
    lastScanBy: sanitizeString(record.lastScanBy),
    updatedAt: normalizeNullable(record.updatedAt),
    updatedBy: sanitizeString(record.updatedBy),
  }
  if (!normalized.hash) return null
  return normalized
}

function rememberCommonTool(record) {
  const normalized = cloneCommonTool(record)
  if (!normalized) return null
  memoryStore.set(normalized.hash, normalized)
  return { ...normalized }
}
...
function memoryGet(hash) {
  ...
  if (!norm) return null
  const stored = memoryStore.get(norm)
  return stored ? { ...stored } : null
}

function memoryList() {
  return Array.from(memoryStore.values()).map(item => ({ ...item }))
}

function memoryUpsert(hash, update = {}, create = {}) {
  const norm = normalizeHash(hash)
  if (!norm) return null
  const existing = memoryStore.get(norm)
  const base = existing ? { ...existing } : { ...create, hash: norm }
  for (const [key, value] of Object.entries(update || {})) {
    if (value !== undefined) {
      base[key] = value
    }
  }
  return rememberCommonTool(base)
}

async function safeCall(scope, fn, fallback) {
  try {
    return await fn()
  } catch (error) {
    console.error(`[commun-data] ${scope} failed`, error)
    return fallback
  }
}
...
async function fetchExisting(hash) {
  if (!hash) return null
  const stored = await safeCall(
    'fetchExisting',
    () => prisma.commonTool.findUnique({ where: { hash } }),
    null
  )
  if (stored) return rememberCommonTool(stored)
  return memoryGet(hash)
}

export async function listTools() {
  const stored = await safeCall(
    'listTools',
    () => prisma.commonTool.findMany(),
    []
  )
  const normalizedStored = []
  for (const tool of stored || []) {
    const remembered = rememberCommonTool(tool)
    if (remembered) {
      normalizedStored.push(remembered)
    }
  }
  const byHash = new Map(normalizedStored.map(tool => [tool.hash, tool]))
  for (const fallback of memoryList()) {
    if (!byHash.has(fallback.hash)) {
      byHash.set(fallback.hash, fallback)
    }
  }
  const results = []
...
export function createToken(hash, actor) {
  const actorId = sanitizeString(actor?.id || '')
  const displayName = sanitizeString(actor?.displayName || actorId)
...
export async function startScan({ hash, name, scannedBy = '', tokenOwner }) {
  ...
  const persisted = stored
    ? rememberCommonTool(stored)
    : memoryUpsert(targetHash, update, createPayload)

  const token = createToken(targetHash, {
    id: sanitizeString(tokenOwner?.id || who || 'scan'),
    displayName: sanitizeString(tokenOwner?.displayName || who || 'scan'),
  })
  const tool = mergeWithBase(base, persisted || existing)
  return { token, tool }
}

export async function patchTool(hash, data, actor) {
  const normHash = normalizeHash(hash)
  if (!normHash) return null
  const base = baseByHash.get(normHash)
  ...
  const now = parisNow()
  const actorId = sanitizeString(actor?.id || '')
  const who = sanitizeString(actor?.displayName || actorId)
  const patch = {}
  ...
  const persisted = stored
    ? rememberCommonTool(stored)
    : memoryUpsert(normHash, update, createPayload)

  const after = mergeWithBase(base, persisted)
  const auditRecord = persisted || existing || createPayload

  audits.push({
    when: auditRecord.updatedAt || now,
